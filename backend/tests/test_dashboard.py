...
def test_get_dashboard_data_success(authorized_client):
    ...
    authorized_client.post(
        f"/events/{event_id}/comments", json={"content": "テストコメント"}
    )
    ...
    assert (
        my_event_summary["attendee_count"] == 1
    )  # 自分の参加が正しくカウントされていること
    assert (
        my_event_summary["comment_count"] == 1
    )  # 自分のコメントが正しくカウントされていること
