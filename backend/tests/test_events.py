...
def test_create_and_get_event(authorized_client):
    ...
    assert "Python" in data["tag_names"]
    assert "id" in data

    # 💡 追記：作成者情報が正しくレスポンスに含まれているか検証
    assert data["creator_name"] == "test_user"
    assert data["creator"] is not None
    assert data["creator"]["username"] == "test_user"

    # 2. イベント一覧 API (GET /events)
    ...
    # 💡 追記：一覧取得でも作成者情報が欠損していないか検証
    assert events_list[0]["creator_name"] == "test_user"
...
def test_cannot_update_or_delete_other_user_event(client, authorized_client):
    ...
    # 💡 追記：他人が作ったイベントであっても、詳細の「取得(GET)」は200で成功することを確認！
    # 💡 かつ、詳細画面で他人の「creator_name (test_user)」がちゃんと見えるかチェック！
    view_res = client.get(f"/events/{event_id}")
    assert view_res.status_code == 200
    assert view_res.json()["creator_name"] == "test_user"
...
def test_event_creator_fallback_when_user_deleted(authorized_client):
    """作成者ユーザーが削除された場合、creator_nameが'不明（退会済み）'になること"""
    start = get_jst_now() + timedelta(days=1)
    end = start + timedelta(hours=2)
    ...
    # データベースセッションを直接開き、作成者である test_user を強制削除する
    from app.database import UserModel, SessionLocal

    db = SessionLocal()
    try:
        user = db.query(UserModel).filter(UserModel.username == "test_user").first()
        if user:
            db.delete(user)
            db.commit()
    finally:
    ...
    # ユーザー削除後、再度イベント詳細APIを叩く
    get_res = authorized_client.get(f"/events/{event_id}")
    assert get_res.status_code == 200
    retrieved_data = get_res.json()

    # 💡 作成者が SET NULL になりつつ、フォールバック文字が返っていることを確認
    assert retrieved_data["creator_id"] is None
    assert retrieved_data["creator"] is None
    assert retrieved_data["creator_name"] == "不明（退会済み）"
